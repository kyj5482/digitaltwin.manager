...
서버가 켜져 있는 동안 **매일 `config.json`의 `autoCommit.time`(기본 23:50)에**
`data/` 변경분을 자동으로 커밋하고 설정된 모든 리모트로 push 한다.
변경이 없으면 커밋하지 않는다. 화면 좌측 하단 칩에서 상태 확인·즉시 실행이 가능하다.
...
// config.json
{
  ...
  "autoCommit": {
    "enabled": true,
    "time": "23:50",            // 매일 실행 시각 (로컬 시간)
    "paths": ["data"],          // 커밋 대상 (코드 변경은 수동 커밋)
    "message": "chore(data): daily snapshot",
    "push": true,
    "remotes": ["origin"]       // push 대상 리모트 목록 — 여러 개 가능
  }
}
```
...
| 방법 | 명령 |
|---|---|
| 즉시 커밋·푸시 | `node server.js --commit-now` 또는 화면의 [지금 커밋·푸시 실행] |
| 서버를 항상 켜두지 않는 경우 (Windows) | 관리자 PowerShell에서 `.\scripts\register-task.ps1` → 작업 스케줄러가 매일 23:50 `autocommit.ps1` 실행 (해제: `schtasks /Delete /TN "NorthPole-AutoCommit" /F`) |

push는 git 리모트 기반이라 GitHub/GitLab을 가리지 않는다. 인증은 Git Credential
Manager 또는 Personal Access Token(GitLab은 `read_repository, write_repository` 스코프).
...
`GET /api/state` · `GET /api/git/status` · `POST /api/git/commit`
`POST /api/goal/save` · `POST /api/milestone/save` · `POST /api/kpi/save|measure|delete` · `POST /api/kpi/sub/save|delete`
`POST /api/project/create`
`POST /api/sprint/create|start|close` · `POST /api/task/create|advance`
`POST /api/request/create|convert|return`
